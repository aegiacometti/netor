...
case $opc in
 y) sudo apt-get update
    ...
    echo ""
    echo "Now:"
    echo "1.- add "/home/netor/netor/" to the end of the /etc/environment file"
    echo "2.- logoff and login again."
    echo "3.- create a new DB with netor-db-switch and populate it with netor-db-customer,"
    echo "netor-db-site and netor-db-devices."
    echo "4.- push it to Ansible and Salt with netor-db-push."
    echo "5.- restart Salt daemons with netor-salt-restart."
    echo "6.- Go to the Use cases section to start trying Ansible and Salt examples."
    echo ""
    ;;
