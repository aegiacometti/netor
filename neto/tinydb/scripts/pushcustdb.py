...
def start_process():
    """
    Checks if the ``neto.conf`` file exist according the local static variable ``NETO_HOME_DIRECTORY``.

    It supports passing as parameter the ``NETO_HOME_DIRECTORY`` in order to push to the DB specified
    in the ``neto.conf`` configuration file.

    Then execute the functions to push the DB content on **Ansible** ans **SaltStack** configuration files, according
    to the settings of the ``neto.conf`` configuration file.
    ...
    **Restart of the Salt minion is required.**
    ...
    try:
        neto_home_directory = sys.argv[1]
    except IndexError:
        neto_home_directory = NETO_HOME_DIRECTORY

    tinydb_log_file = neto_home_directory + "/log/tinydb.log"

    config = configparser.ConfigParser(interpolation=configparser.ExtendedInterpolation())
    print("Reading neto.config")
