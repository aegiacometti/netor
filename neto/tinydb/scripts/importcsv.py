...
def start_process():
    ...
    print('\nAvailable Databases:\n')
    for item in glob.glob(NETO_HOME_DIRECTORY + 'tinydb/data/*.json'):
        print(item)
    ...
    if select_db == '':
        ...
        if not ('.json' in new_db or not new_db.isalnum()):
            print('\nInvalid file name')
            exit(1)
        destination_db_to_import = NETO_HOME_DIRECTORY + 'tinydb/data/' + new_db
        print('\nRemember to \"dbswitch\" in order to start using the data uploaded to the DB.')
    elif select_db in glob.glob(NETO_HOME_DIRECTORY + 'tinydb/data/*.json'):
